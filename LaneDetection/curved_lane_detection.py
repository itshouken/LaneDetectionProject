...
img = mpimg.imread('../calibration/left-0036.png')
img_size1 = (img.shape[1], img.shape[0])


# read the calibration parameters and undistort the image`
#objects = []
with (open("../calibration/NTREX_WebCamInternalParam2.pkl", "rb")) as openfile:
    while True:
        try:
            camera_param = pickle.load(openfile)
        except EOFError:
            break

mtx = camera_param['mtx']
dist = camera_param['dist']
img_size = camera_param['imagesize']
